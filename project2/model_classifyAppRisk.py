...
def merge_data(results):
    ...
    engineered_sparse = coo_matrix(
        engineered_df.astype(np.float32).values
    )
...
def model_data(DATA_FOLDER,MAPPING_FILE,MODEL_OUTPUT,FI_OUTPUT,APP_RISK_DATA_FILE):
    ...
    predictions = (pred >= 0.7).astype(int)
...
def create_arg_parser():
    ...
    parser.add_argument('modelOutputDir',
                        help='Path to the Save tuned Model file.')
    parser.add_argument('outputDir',
                        help='Path to the Save Output feature importance graph.')
    parser.add_argument('intermediateDataFile',
                        help='Path to the Save intermediate data file.')
    return parser
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(parsed_args.mappingFile)):
        ...
        sys.exit(1)
    model_data(os.path.join(parsed_args.inputDirectory),os.path.join(parsed_args.mappingFile),os.path.join(parsed_args.modelOutputDir),os.path.join(parsed_args.outputDir),os.path.join(parsed_args.intermediateDataFile))