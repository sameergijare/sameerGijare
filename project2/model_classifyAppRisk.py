...
def write_to_file(X_sparse,base_feature_names,y):
    ...
    df = df.fillna(0)
    df.to_csv(
        "../input/processed/app_risk_dataset.csv",
        index=False
    )
...
def model_data(DATA_FOLDER,MAPPING_FILE,MODEL_OUTPUT):
    ...
    write_to_file(X_sparse,base_feature_names,y)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        sys.exit(1)
    freeze_support()
    model_data(sys.argv[1],sys.argv[2],sys.argv[3])